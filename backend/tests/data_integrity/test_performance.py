...
from app.models import Archetype, Powerset, Power, Enhancement, Recipe, Salvage
...
class TestDatabasePerformance:
    ...
    def test_pagination_performance(self, db: Session):
        ...
        for page in range(5):
            start = time.time()
            powers = db.query(Power).offset(page * page_size).limit(page_size).all()
            duration = time.time() - start
...
class TestCachePerformance:
    ...
    def test_cache_hit_performance(self, db: Session):
        ...
        start = time.time()
        archetype1 = db.query(Archetype).filter_by(name='Blaster').first()
        miss_time = time.time() - start
        
        # Second query (should hit cache if implemented)
        start = time.time()
        archetype2 = db.query(Archetype).filter_by(name='Blaster').first()
...
class TestQueryOptimization:
    ...
    def test_n_plus_one_detection(self, db: Session):
        ...
        for powerset in powersets:
            ...
            power_count = db.query(Power).filter(
    ...
    def test_query_complexity_limits(self, db: Session):
        ...
        try:
            result = db.query(
                ...
                PowerAllowedEnhancement
            ).outerjoin(
                Enhancement,
                Enhancement.name == PowerAllowedEnhancement.enhancement_type
            ).group_by(
...
class TestScalabilityBenchmarks:
    ...
    def test_concurrent_query_performance(self, db: Session):
        ...
        def run_query(query_id):
            ...
            if query_id % 3 == 0:
                result = db.query(Power).filter(
                    Power.level_available <= 20
                ).limit(50).all()
            elif query_id % 3 == 1:
                result = db.query(Enhancement).filter(
                    Enhancement.enhancement_set_id.isnot(None)
                ).limit(50).all()
            else:
                result = db.query(Recipe).limit(50).all()
