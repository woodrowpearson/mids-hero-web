...
class EffectRequest(BaseModel):
    ...
    effect_type: str = Field(
        ..., description="Effect type (e.g., 'damage', 'defense', 'resistance')"
    )
    magnitude: float = Field(..., description="Base magnitude value")
    duration: float = Field(
        default=0.0, description="Duration in seconds (0 for instant)"
    )
    probability: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Probability of effect occurring"
    )
    damage_type: DamageTypeEnum | None = Field(
        None, description="Damage type for damage effects"
    )
    ticks: int = Field(default=1, ge=1, description="Number of ticks for DoT effects")
    to_who: str = Field(
        default="target", description="Target of effect (self/target/both)"
    )
...
class DamageCalculationRequest(BaseModel):
    ...
    recharge_time: float = Field(
        default=0.0, ge=0.0, description="Base recharge time in seconds"
    )
    cast_time: float = Field(
        default=0.0, ge=0.0, description="Animation/cast time in seconds"
    )
    interrupt_time: float = Field(
        default=0.0, ge=0.0, description="Interrupt time in seconds"
    )
    activate_period: float = Field(
        default=0.0, ge=0.0, description="For toggles, time between ticks"
    )
    ...
    damage_return_mode: DamageReturnModeEnum = Field(
        default=DamageReturnModeEnum.NUMERIC, description="What value to return"
    )
    archetype_damage_cap: float = Field(
        default=4.0, ge=1.0, description="Archetype damage buff cap"
    )
...
class DamageCalculationResponse(BaseModel):
    ...
    has_pvp_difference: bool = Field(
        default=False, description="True if PvE/PvP differ"
    )
    has_toggle_enhancements: bool = Field(
        default=False, description="True if toggle has enhancement effects"
    )
    activate_period: float | None = Field(
        None, description="For toggles, time between ticks"
    )
...
class DefenseBonusInput(BaseModel):
    ...
    bonuses: dict[DefenseTypeEnum, float] = Field(
        ..., description="Defense bonuses by type"
    )

    class Config:
        json_schema_extra = {"example": {"bonuses": {"melee": 0.03, "smashing": 0.02}}}


class ResistanceBonusInput(BaseModel):
    """Resistance bonus from a single source."""

    bonuses: dict[ResistanceTypeEnum, float] = Field(
        ..., description="Resistance bonuses by type"
    )

    class Config:
        json_schema_extra = {"example": {"bonuses": {"smashing": 0.20, "lethal": 0.15}}}
...
class DefenseCalculationRequest(BaseModel):
    ...
    defense_bonuses: list[DefenseBonusInput] = Field(
        default_factory=list, description="List of defense bonuses from all sources"
    )
...
class DefenseCalculationResponse(BaseModel):
    ...
    positional: dict[DefenseTypeEnum, float] = Field(
        ..., description="Positional defense values"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "typed": {"smashing": 0.15, "lethal": 0.10},
                "positional": {"melee": 0.30, "ranged": 0.20, "aoe": 0.15},
                "ddr": 0.0,
                "elusivity": 0.0,
            }
        }
...
class ResistanceCalculationRequest(BaseModel):
    ...
    resistance_bonuses: list[ResistanceBonusInput] = Field(
        default_factory=list, description="List of resistance bonuses from all sources"
    )
...
class ResistanceCalculationResponse(BaseModel):
    ...
    values: dict[ResistanceTypeEnum, float] = Field(
        ..., description="Resistance values by type"
    )
    resistance_debuff_resistance: float = Field(
        default=0.0, description="Resistance to resistance debuffs"
    )

    class Config:
        json_schema_extra = {
            "example": {
                "values": {"smashing": 0.75, "lethal": 0.75, "fire": 0.60},
...
class BuildTotalsRequest(BaseModel):
    ...
    archetype: ArchetypeEnum = Field(..., description="Character archetype")
    defense_bonuses: list[DefenseBonusInput] = Field(
        default_factory=list, description="Defense bonuses from all sources"
    )
    resistance_bonuses: list[ResistanceBonusInput] = Field(
        default_factory=list, description="Resistance bonuses from all sources"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "archetype": "Scrapper",
                "defense_bonuses": [{"bonuses": {"melee": 0.30}}],
                "resistance_bonuses": [{"bonuses": {"smashing": 0.20}}],
            }
        }
...
class BuildTotalsResponse(BaseModel):
    ...
    resistance: ResistanceCalculationResponse = Field(
        ..., description="Resistance totals"
    )
    # TODO: Add recharge, damage, accuracy, and other stats in future updates
...
class GameConstantsResponse(BaseModel):
    ...
    ed_schedule_a_thresholds: list[float] = Field(
        ..., description="Schedule A ED thresholds"
    )
    ed_schedule_b_thresholds: list[float] = Field(
        ..., description="Schedule B ED thresholds"
    )
    ed_schedule_c_thresholds: list[float] = Field(
        ..., description="Schedule C ED thresholds"
    )
    ed_schedule_d_thresholds: list[float] = Field(
        ..., description="Schedule D ED thresholds"
    )
    ...
    invention_origin_l50_value: float = Field(
        ..., description="L50 IO enhancement value"
    )
...
class EnhancementSlotRequest(BaseModel):
    ...
    enhancement_id: int = Field(
        default=-1, description="Enhancement database ID (-1 = empty)"
    )
    ...
    is_catalyzed: bool = Field(
        default=False, description="Superior set (1.25x multiplier)"
    )
    is_boosted: bool = Field(default=False, description="Has enhancement boosters")
    boost_level: int = Field(
        default=0, ge=0, le=5, description="Booster level (+0 to +5)"
    )
...
class ProcCalculationRequest(BaseModel):
    ...
    recharge_time: float = Field(
        ..., ge=0.0, description="Power recharge time in seconds"
    )
    cast_time: float = Field(
        default=0.0, ge=0.0, description="Power cast time in seconds"
    )
    area_factor: float = Field(
        default=1.0, ge=1.0, description="Area modifier (1.0 for single target)"
    )
...
class ProcCalculationResponse(BaseModel):
    ...
    chance_percent: float = Field(
        ..., ge=0.0, le=100.0, description="Proc chance as percentage"
    )
    capped: bool = Field(default=False, description="True if capped at 90%")

    class Config:
        json_schema_extra = {
            "example": {"chance": 0.5625, "chance_percent": 56.25, "capped": False}
        }
