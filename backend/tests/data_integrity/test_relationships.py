...
class TestRelationships:
    ...
    def test_all_foreign_keys_have_indexes(self, db: Session):
        ...
        if issues:
            print(
                f"WARNING: Foreign keys without indexes (performance issue): {issues}"
            )
    ...
    def test_powerset_archetype_relationships(self, db: Session):
        ...
        orphaned = (
            db.query(Powerset)
            .outerjoin(Archetype, Powerset.archetype_id == Archetype.id)
            .filter(Archetype.id.is_(None), Powerset.archetype_id.isnot(None))
            .count()
        )
    ...
    def test_power_powerset_relationships(self, db: Session):
        ...
        orphaned = (
            db.query(Power)
            .outerjoin(Powerset, Power.powerset_id == Powerset.id)
            .filter(Powerset.id.is_(None), Power.powerset_id.isnot(None))
            .count()
        )
    ...
    def test_power_prerequisites_validity(self, db: Session):
        ...
        orphaned_required = (
            ...
            .outerjoin(Power, PowerPrerequisite.required_power_id == Power.id)
            .filter(Power.id.is_(None))
            .count()
        )
        ...
        orphaned_power = (
            db.query(PowerPrerequisite)
            .outerjoin(Power, PowerPrerequisite.power_id == Power.id)
            .filter(Power.id.is_(None))
            .count()
        )
    ...
    def test_enhancement_set_relationships(self, db: Session):
        ...
        orphaned = (
            ...
            .outerjoin(EnhancementSet, Enhancement.set_id == EnhancementSet.id)
            .filter(EnhancementSet.id.is_(None), Enhancement.set_id.isnot(None))
            .count()
        )
    ...
    def test_recipe_enhancement_relationships(self, db: Session):
        ...
        orphaned = (
            db.query(Recipe)
            .outerjoin(Enhancement, Recipe.enhancement_id == Enhancement.id)
            .filter(Enhancement.id.is_(None), Recipe.enhancement_id.isnot(None))
            .count()
        )
    ...
    def test_recipe_salvage_relationships(self, db: Session):
        ...
        orphaned_recipes = (
            db.query(RecipeSalvage)
            .outerjoin(Recipe, RecipeSalvage.recipe_id == Recipe.id)
        ...
        orphaned_salvage = (
            db.query(RecipeSalvage)
            .outerjoin(Salvage, RecipeSalvage.salvage_id == Salvage.id)
            .filter(Salvage.id.is_(None))
            .count()
        )
    ...
    def test_no_orphaned_records(self, db: Session):
        ...
        orphaned_powers = (
            db.query(Power)
            .filter(Power.powerset_id.is_(None), ~Power.name.like("%inherent%"))
            .count()
        )
        ...
        orphaned_powersets = (
            db.query(Powerset)
            .filter(Powerset.archetype_id.is_(None), Powerset.powerset_type != "pool")
            .count()
        )
            ...
            try:
                ...
                for fk in fks:
                    if fk.get("ondelete") != "CASCADE":
                        print(
                            f"Warning: {table_name}.{fk['constrained_columns']} does not have CASCADE delete"
                        )
            except Exception:
...
class TestDataIntegrity:
    ...
    def test_unique_constraints(self, db: Session):
        ...
        duplicate_archetypes = (
            db.query(Archetype.name, func.count(Archetype.id).label("count"))
            .group_by(Archetype.name)
            .having(func.count(Archetype.id) > 1)
            .all()
        )
        ...
        duplicate_powers = (
            db.query(Power.name, Power.powerset_id, func.count(Power.id).label("count"))
            .group_by(Power.name, Power.powerset_id)
            .having(func.count(Power.id) > 1)
            .all()
        )
    ...
    def test_required_fields_not_null(self, db: Session):
        ...
        null_enhancements = (
            db.query(Enhancement)
            .filter((Enhancement.name.is_(None)) | (Enhancement.display_name.is_(None)))
            .count()
        )
    ...
    def test_data_type_constraints(self, db: Session):
        ...
        invalid_levels = (
            db.query(Power)
            .filter((Power.level_available < 1) | (Power.level_available > 50))
            .count()
        )
        ...
        invalid_hp = (
            db.query(Archetype)
            .filter((Archetype.hit_points_base <= 0) | (Archetype.hit_points_max <= 0))
            .count()
        )
    ...
    def test_enum_field_values(self, db: Session):
        ...
        invalid_set_types = (
            db.query(Powerset)
            .filter(~Powerset.powerset_type.in_(valid_set_types))
            .count()
        )
