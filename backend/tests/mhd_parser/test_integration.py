...
from app.mhd_parser.main_database_parser import parse_main_database, MainDatabase
from app.mhd_parser.enhancement_database_parser import parse_enhancement_database, EnhancementDatabase
from app.mhd_parser.salvage_parser import parse_salvage_database, SalvageDatabase
from app.mhd_parser.recipe_parser import parse_recipe_database, RecipeDatabase
from app.mhd_parser.text_mhd_parser import parse_text_mhd, detect_file_format, FileFormat
...
class TestMhdParserIntegration:
    ...
    def test_parse_small_main_database(self):
        ...
        blaster_primary = [ps for ps in db.powersets if ps.archetype_index == 0 and ps.set_type == 0]
    ...
    def test_text_file_integration(self):
        ...
        origins_data = io.BytesIO(b"""Version 1.5.0
Science
Technology
Mutation
Magic
Natural""")
        ...
        levels_data = io.BytesIO(b"""Level\tExperience\tInfluence
1\t0\t0
2\t106\t50
3\t337\t150""")
    ...
    def _write_archetype(self, stream: io.BytesIO, name: str, index: int, class_name: str):
    ...
    def _write_powerset(self, stream: io.BytesIO, name: str, arch_idx: int, 
                       set_type: int, set_name: str):
    ...
    def _write_large_database_sample(self, stream: io.BytesIO):
        ...
        for arch in range(10):
            for set_idx in range(4):
                ...
                self._write_powerset(stream, f"Set_{arch}_{set_idx}", 
                                   arch, set_type, f"Set_{arch}_{set_idx}")
        ...
        for ps in range(40):
            for p in range(5):
                self._write_power(stream, f"Power_{ps}_{p}", 
                                f"Set_{ps // 4}_{ps % 4}", p + 1)
