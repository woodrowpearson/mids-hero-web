...
from app.models import (
    ...
    PowerPrerequisite, PowerEnhancementCompatibility
)
...
class TestGameRules:
        ...
        for inherent in inherent_powersets:
            ...
            for power in powers:
                ...
                if power.name == 'Rest' and power.level_available != 1:
                    issues.append(f"Rest should be available at level 1")
                elif power.name == 'Sprint' and power.level_available != 1:
                    issues.append(f"Sprint should be available at level 1")
    ...
    def test_powerset_archetype_compatibility(self, db: Session):
        ...
        issues = []
        for arch_name, rules in archetype_rules.items():
